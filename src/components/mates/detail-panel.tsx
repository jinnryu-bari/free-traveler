...
function DetailPanelPanel() {
  ...
  const selectedId = readSelectedPostId(
  ...
  const [emptyListCheck, setEmptyListCheck] = useState<{
    key: string;
    isEmpty: boolean;
  }>({
    key: "",
  ...
  useEffect(() => {
    ...
    fetchFilteredMatePosts(filters)
      .then((posts) => {
        if (active)
          setEmptyListCheck({ key: filterKey, isEmpty: posts.length === 0 });
      })
      .catch(() => {
  ...
  const loadPost = useCallback(async (id: string) => {
    try {
      const posts = await fetchFilteredMatePosts({
        country: "",
        region: "",
        start: "",
        end: "",
  ...
  useEffect(() => {
    ...
    fetchFilteredMatePosts({
      ...
      .catch(() => {
        if (active)
          setPostResult({ key: selectedId, post: null, status: "not-found" });
      });
    return () => {
      active = false;
    };
  ...
  const postState: "no-selection" | "loading" | "not-found" | "ready" =
    !selectedId
      ? "no-selection"
  ...
  const loadApplications = useCallback(async (id: string) => {
    try {
      const res = await fetch(`/api/mates/${id}/requests`, {
        cache: "no-store",
      });
  ...
  useEffect(() => {
    ...
    fetch(`/api/mates/${selectedId}/requests`, { cache: "no-store" })
      ...
      .catch(() => {
        if (active)
          setAppsResult({ key: selectedId, applications: null, error: true });
      });
    return () => {
      active = false;
    };
  ...
  const applications =
    appsResult.key === selectedId ? appsResult.applications : null;
  const applicationsError = appsResult.key === selectedId && appsResult.error;

  const closePanel = () => {
    const next = new URLSearchParams(searchParams.toString());
    next.delete(SELECTED_PARAM);
    router.replace(`/mates${next.toString() ? `?${next.toString()}` : ""}`, {
      scroll: false,
    });
  };
  ...
  const respondToRequest = async (
    ...
    showToast(
      "success",
      status === "ACCEPTED" ? "참가 요청을 승인했다" : "참가 요청을 거절했다",
    );
    ...
    return (
      <aside className="border-hairline hidden rounded-md border p-8 text-center lg:block lg:w-[60%]">
        <p className="text-body-md text-body">
          왼쪽 목록에서 동행글을 선택하면 상세 정보가 여기에 표시됩니다.
        </p>
      </aside>
    );
  }
  ...
  return (
    <aside className="border-hairline fixed inset-x-0 bottom-0 z-40 max-h-[80vh] overflow-y-auto rounded-t-md border bg-surface p-5 lg:static lg:z-auto lg:max-h-none lg:w-[60%] lg:overflow-visible lg:rounded-md">
      <div className="mb-3 flex items-center justify-between">
        <span
          aria-hidden
          className="mx-auto block h-1 w-10 rounded-full bg-surface-strong lg:hidden"
        />
        <button
          type="button"
          onClick={closePanel}
          className="text-body-sm text-body ml-auto hover:text-ink"
        >
          닫기
        </button>
      </div>
      ...
      {postState === "not-found" && (
        <p className="text-body-md text-ink">
          동행글을 찾을 수 없습니다. 삭제되었거나 접근할 수 없는 글입니다.
        </p>
      )}
        ...
        <div className="flex flex-col gap-4">
          ...
          {isAuthor ? (
            <div className="border-hairline rounded-md border p-4">
              ...
              <p className="text-body-sm text-body mt-1">
                이 글은 본인이 작성했습니다. 참가 신청 대신 아래에서 승인/거절과
                마감을 관리할 수 있습니다.
              </p>
              ...
              <div className="mt-4 flex flex-col gap-2">
                ...
                {!applicationsError &&
                  applications?.map((app) => (
                    <div
                      key={app.id}
                      className="border-hairline flex items-center justify-between gap-2 rounded-sm border p-3"
                    >
                      <div>
...
export function DetailPanel() {
  return (
    <Suspense
      fallback={
        <div className="h-64 animate-pulse rounded-md bg-surface-strong lg:w-[60%]" />
      }
    >
      <DetailPanelPanel />
    </Suspense>
