...
export function readSelectedPostId(
...
function ListGridPanel() {
  ...
  const visibleCount =
    visible.key === filterKey ? visible.count : INITIAL_VISIBLE;
  ...
  const detailPanelVisible =
    Boolean(selectedId) || (posts !== null && posts.length > 0);

  useEffect(() => {
    const authorIds = Array.from(
      new Set(visiblePosts.map((p) => p.author_id)),
    ).filter((id) => !(id in nicknames));
      ...
      {!loadError && posts === null && (
        <div className="grid grid-cols-1 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-28 animate-pulse rounded-md bg-surface-strong"
            />
          ))}
        </div>
      )}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <p className="text-title-md text-ink line-clamp-1">
                    {post.title}
                  </p>
                  <span
                ...
                <p className="text-body-sm text-body">
                  모집 인원 {post.capacity}명 · 작성자{" "}
                  {nicknames[post.author_id] ?? "..."}
                </p>
              </button>
            );
          })}
          ...
          {posts.length > visibleCount && (
            <button
              ...
              onClick={() =>
                setVisible({
                  key: filterKey,
                  count: Math.min(visibleCount + INITIAL_VISIBLE, MAX_VISIBLE),
                })
              }
              className="text-button border-hairline inline-flex h-11 items-center justify-center rounded-sm border px-5 text-ink hover:bg-surface-soft"
            >
...
export function ListGrid() {
  return (
    <Suspense
      fallback={
        <div className="h-64 w-full animate-pulse rounded-md bg-surface-strong lg:w-[40%]" />
      }
    >
      <ListGridPanel />
    </Suspense>
