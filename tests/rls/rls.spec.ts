...
describe.skipIf(!HAS_SUPABASE_ENV)("TEST-RLS-BASIC — 기본 RLS 통합 테스트", () => {
  describe.skipIf(!HAS_ACCOUNTS_CONFIGURED)("로그인 필요 케이스(.env.test.local 준비됨)", () => {
    beforeAll(async () => {
    ...
    it("moderator는 정책 예외로 mate_applications를 조회할 수 있다(RLS 거부 없음)", async () => {
      const { error } = await moderatorClient.from("mate_applications").select("id").limit(1);
      expect(error).toBeNull();
    });
  });

  it(
    "비로그인(anon)은 mate_applications를 조회할 수 없다(GRANT 자체가 없음, 로그인 불필요)",
    async () => {
      const anon = createClient(SUPABASE_URL!, ANON_KEY!);
      const { data, error } = await anon.from("mate_applications").select("*").limit(1);
      if (!error) {
