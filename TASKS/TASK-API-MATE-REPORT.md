...
이 Task는 `design-reference/SCREEN_ROUTE_CONTRACT.json`의 technical_routes에 정의된 API Route `/api/mates/[id]/report`를 구현한다. UI Component가 이 API를 호출해 데이터를 읽고 쓴다.
...
- 신고 대상에게 신고자 식별정보 미노출

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 사유 코드+설명 저장, 신고 ID·접수 시각 응답(목표 p95 3초 이내)
