...
'관리자 외부 URL 설정'는 SCR-005 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`API-ADMIN-EXTERNAL-LINKS` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- `src/components/account/admin-external-links.tsx`

**이 Task는 위에 나열된 파일(들) 밖의 어떤 파일도 만들거나 수정하지 않는다.** 다른 Task의 Expected Files와 겹치는 파일을 이 Task에서 건드리지 않는다.

## Functional AC

- 항공·호텔 외부 URL을 허용목록 내 HTTPS로만 저장, 저장 성공 시 인라인 확인 메시지
...
- HTTP·javascript·data URL 저장 거부

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 항공·호텔 외부 URL을 허용목록 내 HTTPS로만 저장, 저장 성공 시 인라인 확인 메시지
