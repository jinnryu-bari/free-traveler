# MANUAL-PERF-LIGHTHOUSE — Lighthouse 수동 실행

**Category:** MANUAL_CHECK
**Implementation Status:** IMPLEMENT(best-effort)
**Priority:** P2
**Seq:** 72

## Context

'Lighthouse 수동 실행'는 자동화하지 않고 수동 점검·체크리스트 서명으로 완료를 기록하는 Task다(`docs/PROJECT_SCOPE.md` §2 최종 확인 규칙).

## Project Scope

**Implementation Status:** `IMPLEMENT(best-effort)` — `docs/PROJECT_SCOPE.md`가 best-effort로 명시(자동 CI 게이트 없이 목표 지향).
...
- 중급 모바일/4G 기준 LCP/INP/CLS 목표치 best-effort 측정 및 기록(자동 CI 게이트 아님)
