...
- 모든 API Route가 이 레이어를 통해서만 DB 접근, 입력 검증(zod 등)으로 저장 XSS 차단
...
- CSRF/SameSite 쿠키 기본값 사용, 비밀키는 환경변수로만 참조

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 모든 API Route가 이 레이어를 통해서만 DB 접근, 입력 검증(zod 등)으로 저장 XSS 차단
