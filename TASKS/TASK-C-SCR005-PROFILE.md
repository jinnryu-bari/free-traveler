...
'Member 프로필 Form + 성인확인 배지'는 SCR-005 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`DB-SCHEMA-BASE` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- 닉네임·연령대·여행스타일 필수, 성별 선택, 성인확인 상태 요약 배지("성인 인증 완료 · YYYY-MM-DD")

## Visual AC

- Desktop Form 2열, Mobile 1열

## Security/Privacy AC

- 정확한 생년월일 미저장, `is_adult`+`adult_verified_at`만 저장

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 닉네임·연령대·여행스타일 필수, 성별 선택, 성인확인 상태 요약 배지("성인 인증 완료 · YYYY-MM-DD")
