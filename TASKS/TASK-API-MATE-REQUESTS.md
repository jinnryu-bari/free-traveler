...
이 Task는 `design-reference/SCREEN_ROUTE_CONTRACT.json`의 technical_routes에 정의된 API Route `/api/mates/[id]/requests`를 구현한다. UI Component가 이 API를 호출해 데이터를 읽고 쓴다.
...
|---|---|
| Screen | SCR-004 |
| Route | `/api/mates/[id]/requests` |
| Page Entry | `src/app/api/mates/[id]/requests/route.ts` |
...
- `src/app/api/mates/[id]/requests/route.ts`

**이 Task는 위에 나열된 파일(들) 밖의 어떤 파일도 만들거나 수정하지 않는다.** 다른 Task의 Expected Files와 겹치는 파일을 이 Task에서 건드리지 않는다.

## Functional AC

- POST: 중복 PENDING/ACCEPTED 차단(DB unique+애플리케이션 검증)
- PATCH: 작성자만 승인/거절(비작성자 403)
- 종료일 경과 글은 조회 시 CLOSED로 표시
...
- 참가 메시지는 작성자·요청자만 조회 가능

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: POST: 중복 PENDING/ACCEPTED 차단(DB unique+애플리케이션 검증); PATCH: 작성자만 승인/거절(비작성자 403); 종료일 경과 글은 조회 시 CLOSED로 표시
