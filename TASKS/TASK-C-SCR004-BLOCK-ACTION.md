...
'차단 실행 버튼'는 SCR-004 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`API-BLOCKLIST` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- 차단 상태는 RLS로 서버 측에서도 강제

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 차단 실행 후 상호 글·프로필·요청 노출 즉시 차단, 차단 해제는 SCR-005에서 관리
