...
'동행 모집글 작성 Form(인증 게이트)'는 SCR-003 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`API-MATES; C-GLOBAL-POLICY-SAFETY-GUIDE` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
| Screen | SCR-003 |
| Route | `/travel-tools` |
| Page Entry | `src/app/travel-tools/page.tsx`(사용처) |
...
- `design-reference/D-001/DESIGN.md` §10 Form·Tabs, §13 Alert·Toast(비전달 고지)
...
- `src/components/travel-tools/mate-composer-form.tsx`
- `src/lib/validation/contact-detection.ts`

**이 Task는 위에 나열된 파일(들) 밖의 어떤 파일도 만들거나 수정하지 않는다.** 다른 Task의 Expected Files와 겹치는 파일을 이 Task에서 건드리지 않는다.

## Functional AC

- 미인증/미성년 시 로그인·성인확인 안내 CTA Banner로 폼 자체를 렌더하지 않음
- 인증 완료 시 제목·국가·기간·모집인원·스타일·설명·안전수칙 동의 Form(필수값·날짜역전·과거종료일 차단)
- 본문 전화번호/이메일/메신저ID 패턴 탐지 시 제출 차단+수정 안내

## Visual AC

- Desktop/Mobile Form 1열 공통

## Security/Privacy AC

- 연락처 패턴 탐지율 기준 테스트셋 95%↑/오탐 5%↓ 목표, 안전수칙 동의(버전·시각) 기록

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 미인증/미성년 시 로그인·성인확인 안내 CTA Banner로 폼 자체를 렌더하지 않음; 인증 완료 시 제목·국가·기간·모집인원·스타일·설명·안전수칙 동의 Form(필수값·날짜역전·과거종료일 차단); 본문 전화번호/이메일/메신저ID 패턴 탐지 시 제출 차단+수정 안내
- `UNIT-CONTACT-DETECTION`(연락처 탐지 단위 테스트)가 이 Task의 완료를 검증한다 — 해당 Task 실행 시 본 Task의 Functional/Security AC가 자동/수동으로 재확인되어야 한다.
