...
'국가 안전정보 정적 데이터'는 콘텐츠를 `src/data/` 정적 TypeScript 데이터로 관리하는 Data Task다(`docs/PROJECT_SCOPE.md` §2 원칙 — CMS 없음). 이 데이터는 관련 Component/Page Owner Task가 읽기 전용으로 소비한다.
...
`REQ-FUNC-046,047,048,052,053; REQ-NF-027`
...
- 게시되는 모든 해외 국가에 안전정보 1건 이상, 8개 카테고리(치안/사기/법규/교통/재난·기후/보건/문화·복장/긴급연락처) 스키마 강제, 출처명·URL·최종확인일·편집자 필드, `scope_type`/`scope_text`
...
- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 게시되는 모든 해외 국가에 안전정보 1건 이상, 8개 카테고리(치안/사기/법규/교통/재난·기후/보건/문화·복장/긴급연락처) 스키마 강제, 출처명·URL·최종확인일·편집자 필드, `scope_type`/`scope_text`
