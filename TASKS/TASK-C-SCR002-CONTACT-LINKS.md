...
'문의·SNS 링크'는 SCR-002 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`DATA-REPRESENTATIVE` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 관리자 설정 기반 링크, 빈 값은 렌더링하지 않음, 허용 프로토콜(mailto/https)만 오픈
