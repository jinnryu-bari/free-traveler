...
'방문 국가 Chip(권역별)'는 SCR-002 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`DATA-REPRESENTATIVE` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- `design-reference/D-001/DESIGN.md` §8 Search·Filter (chip), §5 Radius(pill)
...
- `src/components/about/country-chips.tsx`

**이 Task는 위에 나열된 파일(들) 밖의 어떤 파일도 만들거나 수정하지 않는다.** 다른 Task의 Expected Files와 겹치는 파일을 이 Task에서 건드리지 않는다.

## Functional AC

- 최소 30개국, 권역별(아시아/유럽/북미/오세아니아 등) 그룹 헤딩, 관련 여행지 있으면 SCR-001 이동·없으면 비활성 스타일

## Visual AC

- Desktop 권역별 가로 배치, Mobile 세로 스택
...
- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 최소 30개국, 권역별(아시아/유럽/북미/오세아니아 등) 그룹 헤딩, 관련 여행지 있으면 SCR-001 이동·없으면 비활성 스타일
