...
'기억에 남는 여행지 + CTA Banner'는 SCR-002 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`DATA-DESTINATIONS` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- `src/components/about/memorable-cta.tsx`

**이 Task는 위에 나열된 파일(들) 밖의 어떤 파일도 만들거나 수정하지 않는다.** 다른 Task의 Expected Files와 겹치는 파일을 이 Task에서 건드리지 않는다.

## Functional AC

- 카드 4개, 비공개 여행지는 자동 제외+대체 후보, 하단 CTA Banner 버튼 2개(`/travel-tools`,`/mates`)

## Visual AC

- Desktop 카드+Banner, Mobile 1열 스택+Banner
...
- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 카드 4개, 비공개 여행지는 자동 제외+대체 후보, 하단 CTA Banner 버튼 2개(`/travel-tools`,`/mates`)
