...
- Chromium 단일 브라우저로 ①항공 조건 입력→요약→외부 이동 새 탭 오픈 확인(네트워크 탭에 목적지·날짜 query 없음 assert), ②숙소 조건 동일 흐름, 2개 핵심 흐름 happy path만 검증
...
- 입력값이 어떤 네트워크 요청 body/query에도 나타나지 않음을 자동 assert
