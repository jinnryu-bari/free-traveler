...
이 Task는 `design-reference/SCREEN_ROUTE_CONTRACT.json`의 technical_routes에 정의된 API Route `/api/admin/reports`를 구현한다. UI Component가 이 API를 호출해 데이터를 읽고 쓴다.
...
| Page Entry | `src/app/api/admin/reports/route.ts` |
...
- GET 필터(OPEN/RESOLVED/DISMISSED), PATCH 상태 변경, Moderator/Admin 외 403
...
- role 서버 재검증(클라이언트 role 신뢰 금지)

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: GET 필터(OPEN/RESOLVED/DISMISSED), PATCH 상태 변경, Moderator/Admin 외 403
