...
이 Task는 `/mates`(SCR-004) 화면의 Page Owner다. 이미 완료된 하위 Component/Data/API Task들이 만든 조각을 실제 Route Page(`src/app/mates/page.tsx`)에 조립하는 것만 범위이며, 새로운 Component를 이 Task 안에서 직접 구현하지 않는다(§UI_CONTRACT.md, D-001 §18 Section 순서 기준).
...
- `C-SCR004-INTRO-CTA` — Intro CTA Banner
- `C-SCR004-FILTER-SUMMARY` — Filter + 결과 요약
- `C-SCR004-LIST-GRID` — 동행글 목록 Card Grid
- `C-SCR004-DETAIL-PANEL` — 목록+상세 Split/Drawer, 작성자 액션
- `C-SCR004-PARTICIPATE-FLOW` — 참가 요청 제출(중복 차단)
- `C-SCR004-REPORT-MODAL` — 신고 버튼/모달
- `C-SCR004-BLOCK-ACTION` — 차단 실행 버튼
- `C-SCR004-STEPS-SAFETY` — 신청 방법 3단계 + 안전 안내 CTA
- `API-MATES` — 동행글 목록·생성 API
- `API-MATE-REQUESTS` — 참가 요청·승인/거절 API
- `API-MATE-REPORT` — 신고 접수 API
...
- Section 순서를 Intro→Filter·결과 요약→동행 목록→상세→신청 방법 3단계→안전·신고·차단 안내와 CTA 순으로 조립
- 데이터 출처는 API(`/api/mates`, `/api/mates/[id]/requests`)
- 목록 최대 8개 카드 우선 노출+"더 보기", 결과 0건이어도 조건 완화 안내+CTA 2개(필터 초기화/작성하기)

## Visual AC

- Desktop 좌40%목록+우60%상세 Split 고정, Mobile 목록 1열+하단 Drawer 상세

## Security/Privacy AC

- 카드·상세 어디에도 이메일·전화번호·메신저 ID를 렌더링하지 않음(REQ-FUNC-033)
- 참가/승인/신고/차단은 인증된 세션에서만 서버 검증

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: Section 순서를 Intro→Filter·결과 요약→동행 목록→상세→신청 방법 3단계→안전·신고·차단 안내와 CTA 순으로 조립; 데이터 출처는 API(`/api/mates`, `/api/mates/[id]/requests`); 목록 최대 8개 카드 우선 노출+"더 보기", 결과 0건이어도 조건 완화 안내+CTA 2개(필터 초기화/작성하기)
