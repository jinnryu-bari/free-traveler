...
'Hero 검색 입력 로직 연결'는 SCR-001 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`DATA-DESTINATIONS` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- `design-reference/D-001/DESIGN.md` §8 Search·Filter, §16 Hero 높이 규칙
...
- 기존 정적 검색 인풋에 부분 일치 키워드 검색·빈 결과 상태를 연결하고 필터 상태를 URL query로 동기화·복원
...
- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 기존 정적 검색 인풋에 부분 일치 키워드 검색·빈 결과 상태를 연결하고 필터 상태를 URL query로 동기화·복원
- `UNIT-TRAVEL-DATES(해당없음)`(UNIT-TRAVEL-DATES(해당없음))가 이 Task의 완료를 검증한다 — 해당 Task 실행 시 본 Task의 Functional/Security AC가 자동/수동으로 재확인되어야 한다.
