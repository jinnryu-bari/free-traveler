...
'소개·철학 Split'는 SCR-002 화면의 UI_CONTRACT.md 주요 Component 목록에 정의된 조각을 구현하는 Component Task다. Page Owner Task(`DATA-REPRESENTATIVE` 중 PAGE-* 항목)가 이 Task의 산출물을 조립한다.
...
- `src/components/about/intro-philosophy.tsx`

**이 Task는 위에 나열된 파일(들) 밖의 어떤 파일도 만들거나 수정하지 않는다.** 다른 Task의 Expected Files와 겹치는 파일을 이 Task에서 건드리지 않는다.

## Functional AC

- 자기소개/여행을 시작한 이유/여행 철학 목차형 소제목 + 2~4문단 본문

## Visual AC

- Desktop 좌목차+우본문, Mobile 세로 스택
...
- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 자기소개/여행을 시작한 이유/여행 철학 목차형 소제목 + 2~4문단 본문
