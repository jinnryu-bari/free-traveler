...
이 Task는 `design-reference/SCREEN_ROUTE_CONTRACT.json`의 technical_routes에 정의된 API Route `/auth/callback`를 구현한다. UI Component가 이 API를 호출해 데이터를 읽고 쓴다.
...
- 가입/로그인/비밀번호 재설정 리다이렉트 처리, 세션 발급 후 `/account`로 복귀
...
- 인증되지 않은 이메일은 동행 쓰기 권한 미부여

## Test Cases

- Functional AC의 각 조건을 개별 케이스로 분리해 확인한다: 가입/로그인/비밀번호 재설정 리다이렉트 처리, 세션 발급 후 `/account`로 복귀
